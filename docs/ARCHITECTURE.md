...
- **rules/analyzers** apply the heuristics shipped in `reposecure/core.py`.
- **score** ranks by severity.
- **MCP server** (`reposecure mcp`) exposes `scan` for Cognis.Studio agents.
