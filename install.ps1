# Comprehensive installer for cognis-digital/reposecure (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# reposecure is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "reposecure"
$Url  = "git+https://github.com/cognis-digital/reposecure.git"
$Git  = "https://github.com/cognis-digital/reposecure.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: reposecure"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: reposecure"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: reposecure"; exit 0 }
}
