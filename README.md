...
[![PyPI](https://img.shields.io/pypi/v/cognis-reposecure.svg)](https://pypi.org/project/cognis-reposecure/)
[![CI](https://github.com/cognis-digital/reposecure/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/reposecure/actions)
...
REPOSECURE exists for one job — one-shot repo security posture grade (secrets/ci/branch rules/deps) — and does it without a SaaS bill or heavyweight setup.
...
```bash
reposecure --version
reposecure scan .                      # scan the current project
reposecure scan . --format json
reposecure scan . --fail-on high       # non-zero exit for CI gates
reposecure mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **scorecard**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`reposecure` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
